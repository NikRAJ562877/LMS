...
export const StudentResources = () => {
    const { notes, students } = useData();
    const { user } = useAuth();

    // Get current student details to filter notes
    const currentStudent = students.find(s => s.id === user?.id);

    const filteredNotes = notes.filter(note => {
        if (!currentStudent) return false;

        const matchClass = note.classLevel === currentStudent.classLevel;
        const matchBatch = note.batch === 'all' || note.batch === currentStudent.batch;
            ...
            <div className="bg-white rounded-lg shadow p-6">
                ...
                {filteredNotes.length > 0 ? (
                    ...
                    <div className="text-center py-12 text-gray-500">
                        ...
                        <p>No resources available for your class yet.</p>
                    </div>
                )}
            </div>
        </div>
